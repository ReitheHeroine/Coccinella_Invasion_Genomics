...
def parse_location_info(raw: str):
    ...
    s_wo_coords = re.sub(r'\(' + re.escape(coord_text) + r'\)\s*', '', s) if coord_text else s
...
def main():
    ap = argparse.ArgumentParser(
        description='Create samples.tsv and population files from Excel metadata.')
    ...
    with open(log_path, 'w', encoding='utf-8') as log:
        ...
        log_print("Running create_metadata.py")
        ...
        try:
            ...
            raise SystemExit(f"ERROR: Failed to read Excel file '{args.excel}': {e}") from e
        ...
        for col in (args.sample_col, args.locinfo_col, args.continent_col):
            if col not in df.columns:
                log_print(f"ERROR: expected column '{col}' not found. "
        ...
        required_cols = [
            'sample_id', 'population', 'country', 'city',
            'latitude', 'longitude', 'continent',
        ]
        samples[required_cols].to_csv(samples_path, sep='\t', index=False)
        ...
        if not created_pop_files:
            log_print("WARNING: No population-specific pop_*.txt files were created "
                      "(no non-NA groups?).")
        ...
        # missing rows
        missing_mask = samples[required_cols].isna().any(axis=1)
        missing_rows = samples.loc[missing_mask]
        if len(missing_rows) > 0:
            missing_rows.to_csv(missing_path, sep='\t', index=False)
            log_print(f"Wrote rows with missing required fields: {missing_path} "
                      f"(n={len(missing_rows)})")
        else:
